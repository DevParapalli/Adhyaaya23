<script lang="ts">
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        const elem = interactable.closest('a, button, input, textarea, select, details, summary, [contenteditable="true"], .interactable, .cursor-pointer, .cursor-pointer *, span, img')
        if (elem !== null) {
            interacting = true;
        } else {
            interacting = false;
...
<div id="mouse-trailer" class="mouse-trailer flex items-center justify-center text-xl pointer-events-none" bind:this={trailerRef}>
    <iconify-icon class="icon transition-all duration-500 ease-in-out text-black" icon={icon} />
</div>
