	...
	<form use:enhance use:autoanimate class="registration-form container max-w-3xl mx-auto px-10 py-10 bg-black/50" method="POST">
		...
		{#if teamMemberSelected > 0}
			<h4
				...
				class="mb-2 text-base font-medium text-gray-900 dark:text-gray-200"
			>
...
<style>
	.bg {
		background: linear-gradient(0deg, rgba(0, 0, 0, var(--opacity)), rgba(0, 0, 0, var(--opacity))) center fixed no-repeat, var(--bg) center fixed no-repeat;
	}
</style>